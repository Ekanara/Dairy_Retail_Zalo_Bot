"""
Structured JSON logger for prompt-service.
...
Usage:
    ...
    logger.info("prompt.rendered", user_id="zalo_123", latency_ms=42)
    logger.error("db.error", user_id="zalo_123", error=str(exc))
"""
...
class _ContextLogger(logging.LoggerAdapter):
    ...
    Thin wrapper that lets callers pass structured fields as kwargs:
        ...
        logger.info("prompt.rendered", user_id="x", latency_ms=12)
...
def get_logger(name: str) -> _ContextLogger:
    ...
    -------
    logger = get_logger(__name__)
    logger.info("user.init", user_id="123", char_delta=0)
    """
